...
- 내가 "이렇게 하는 거 어때?"와 같이 의견을 물을 때, 동의하지 않거나 더 나은 방법이 있다면 긍정적으로만 반응하지 말고 반대 의견이나 문제점을 가감 없이 제시해라.<br><br>
...
- 시니어 엔지니어가 "이거 왜 이렇게 복잡해?"라고 할 것 같으면, 단순화해라.<br><br>
...
- 코드 내 변수명, 함수명, 클래스명 등의 식별자는 영문으로 작성한다.<br><br>

---

## 아키텍처

상세 구조: `rules/architecture.md`<br><br>
...
- 엔티티 `@Setter` 금지, 비즈니스 메서드로 상태 변경<br><br>
...
- Service 레이어가 예외 처리의 주 책임<br><br>
...
- `rules/commit.md` — 커밋 규칙 변경<br><br>
